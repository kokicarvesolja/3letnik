...
V shemi instrumentacijskega ojačevalnika se uporablja tudi negativno povratno zank s shemo 
...
#+attr_latex: :width 0.5\textwidth :caption \caption{Vezje, s katerim preučujemo termični šum na uporu, vir \cite{likar_osnove_2016}.}
...
Disperzija brez členov za merjenje je
...
\[ P = \frac{Q}{2 \tau}, 
\]
ki jo lahko povežemo z ekviparticijskim izrekom z eno prostostno stopnjo
...
\[ \frac{\mathrm{d} \left\langle U ^2 \right\rangle}{\mathrm{d} \omega} = \frac{1}{\pi} \int\limits_{- \infty}^{\infty} 2 k TR \delta(t) e^{- \mathrm{i} \omega t}  \, \mathrm{d} t = 2 \frac{k T R}{\pi} = \phi(\omega)
\]
