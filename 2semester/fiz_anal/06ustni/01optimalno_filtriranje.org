...
združimo v \( \left( \bar{z}_3, \sigma_3 \right) \) po definiciji povprečja

\[ \bar{z}_3 = \frac{1}{N + M} \sum\limits_{i = 1}^{N + M} z_i, \ \sigma_3 ^2 = \frac{\sigma ^2}{N + M}.
\]
...
\end{equation}
kjer sta \( \Phi_n, \Gamma_n \) matriki, \( \mathbf{c}_n \) znan vektor in \( \mathbf{w}_n \) naključen vektor s povprečjem nič in kovariančno matriko \( \left\langle \mathbf{w}_n \mathbf{w}_{n'}^T \right\rangle = Q_n \delta_{n, n'}\). Pri prehodu iz \( n \)-te meritve na \( (n + 1) \) meritev najprej zapišemo oceno
...
V zvezni sliki je dinamična enačba

\begin{equation}
\label{eq:20}
\dot{\hat{\mathbf{x}}} = A(t) \mathbf{x} + \mathbf{c}(t) + \Gamma \mathbf{w}.
\end{equation}

Pri prehodu na zvezno sliko je optimalna shema za sledenje zvezni spremenljivki

\[ \dot{\hat{\mathbf{x}}} (t) = A(t) \hat{\mathbf{x}} + \mathbf{c}(t) + \mathsf{P} \mathsf{H}^T \mathsf{R}^{-1} \left( \mathbf{z} - \mathsf{H} \hat{\mathbf{x}}(t) \right), 
\]
kjer je \( A(t) = \lim_{T \to 0}  \frac{\Phi_{n + 1} - 1}{T} \) in \( \mathbf{c}(t) = \lim_{T \to 0} \frac{\mathbf{c}_n}{T} \).

Kovariančna matrika izostrene ocene je

\[ \dot{\mathsf{P}} = A \mathsf{P} + \mathsf{P} A^T + \Gamma Q \Gamma^T - \mathsf{P} \mathsf{H}^T \mathsf{R}^{-1} \mathsf{H} \mathsf{P}
\]
...
\begin{align}
 \label{ali:1} \mathsf{A} + \mathsf{B} \mathsf{H} &= \mathsf{I} \\ \label{ali:2}
\mathsf{A} \mathbf{m} + \mathsf{B} \mathbf{r} &= \mathbf{p}, 
\end{align}
...
\end{align*}
kjer smo v drugi vrstici izpostavili \( \mathsf{H}^T \). Z upoštevanjem \( \mathsf{I} = \mathsf{R}^{-1} \mathsf{R} \) pa lahko izpostavimo še \( \mathsf{R}^{-1} \)

\[ \mathsf{P} \mathsf{H}^T \mathsf{R}^{-1} \left( \mathsf{R} + \mathsf{H} \mathsf{M} \mathsf{H}^T \right) = \mathsf{M} \mathsf{H}^T.
\]

Nadalje upoštevamo

\[ \mathsf{P} \mathsf{H}^T \mathsf{R}^{-1} = \mathsf{M} \mathsf{H}^T \left( \mathsf{R} + \mathsf{H} \mathsf{M} \mathsf{H}^T \right)^{-1} 
\]
v enačbi \eqref{eq:16} in dobimo iskano zvezo

\[ \mathsf{P} = \mathsf{M} - \mathsf{M} \mathsf{H}^T \left( \mathsf{H} \mathsf{M} \mathsf{H}^T + \mathsf{R} \right)^{-1} \mathsf{H} \mathsf{M}. 
\]
#+end_izpeljava

#+attr_latex: :options [Diskretne in zvezne meritve]
#+begin_izpeljava
Ocena meritev ob \( n + 1 \) meritvi je

\begin{equation}
\label{eq:18}
 \bar{\mathbf{x}}_{n + 1} = \Phi_n \hat{\mathbf{x}} + \mathbf{c}_n, 
\end{equation}
po definiciji pa izračunamo tudi kovariančno matriko

\begin{align*}
  \mathsf{M}_{n + 1} &= \left\langle \left( \bar{\mathbf{x}}_{n + 1} - \mathbf{x}_{n + 1} \right) \left( \bar{\mathbf{x}}_{n + 1} - \mathbf{x}_{n + 1} \right)^T \right\rangle \\
&= \left\langle \left[ \Phi_n \left( \hat{\mathbf{x}}_n - \mathbf{x}_n \right) - \Gamma_n \mathbf{w}_n \right] \left[ \Phi_n \left( \hat{\mathbf{x}}_n - \mathbf{x}_n\right) - \Gamma_n \mathbf{w}_n \right]^T \right\rangle \\ 
&= \left\langle \Phi_n \left( \hat{\mathbf{x}}_n - \mathbf{x}_n \right) \left( \hat{\mathbf{x}}_n - \mathbf{x}_n \right)^T \Phi^T  \right\rangle + \Gamma_n \left\langle \mathbf{w}_n \mathbf{w}_{n'} \right\rangle \Gamma_n^T \\
&= \Phi_n P_n \Phi_n^T + \Gamma_n Q_n \Gamma_n^T,
\end{align*}
kjer smo iz prve v drugo vrstici upoštevali definicijo ocene \eqref{eq:18} ter definicijo dinamike \eqref{eq:19}. Iz prejšnje izpeljave sledi kovariančna matrika izostrene ocene

\[ \mathsf{P}_{n + 1} = \mathsf{M}_{n + 1} - \mathsf{M}_{n + 1} \mathsf{H}^T \left( \mathsf{H} \mathsf{M}_{n + 1}^T + \mathsf{R}_{n + 1}^{-1} \right) \mathsf{H} \mathsf{M}_{n + 1}.
\]

V limiti časovne periode \( T \to 0 \), podobno kot pri skalarni spremenljivki zapišemo

\[ \lim_{T \to 0}  \frac{\hat{\mathbf{x}}_{n + 1} + \hat{\mathbf{x}}_n}{T} = \dot{\hat{\mathbf{x}}} (t) = \lim_{T \to 0} \frac{\bar{\mathbf{x}}_{n + 1} - \mathsf{P}_{n + 1} \mathsf{H}^T \mathsf{R}^{-1}_{n + 1} \left( \mathbf{z}_{n + 1} - \mathsf{H} \bar{\mathbf{x}}_{n + 1} \right) - \hat{\mathbf{x}}_n}{T}. 
\]
S pomočjo enačbe \eqref{eq:18} zapišemo

\[ \dot{\hat{\mathbf{x}}} (t) = \lim_{T \to 0} \frac{\Phi_n - 1}{T} \hat{\mathbf{x}}_n + \frac{\mathbf{c}_n}{T} + \frac{K_{n + 1}}{T} \left( \mathbf{z}_{n + 1} - \mathsf{H} \bar{\mathbf{x}}_{n + 1} \right).
\]
Označimo

\begin{align}
 \label{ali:3} \lim_{T \to 0} \frac{\Phi_n - 1}{T} &= A(t)  \\ 
\lim_{T \to 0} P_{n + 1} &= P(t) \\
\lim_{T \to 0} \left( R^{-1} T \right) &= R(t) \\
\lim_{T \to 0} K_{n + 1} &= K(t),
\end{align}
in optimalna shema za sledenje zvezni spremenljivki je

\[ \dot{\hat{\mathbf{x}}} (t) = A(t) \hat{\mathbf{x}} + \mathbf{c}(t) + K(t) \left[ \mathbf{z}(t) - \mathsf{H} \bar{\mathbf{x}} \right].
\]

Za kovariančno matriko pa v zvezni sliki najprej zapišemo razliko

\[ \mathsf{P}_{n + 1} - \mathsf{P}_n = \mathsf{M}_{n + 1} - \mathsf{M}_{n + 1} \mathsf{H}^T \left( \mathsf{H} \mathsf{M}_{n + 1}^T + \mathsf{R}_{n + 1} \right)^{-1} \mathsf{H} \mathsf{M}_{n + 1} - \mathsf{P}_n, 
\]
v kateri upoštevamo definicijo kovariančne matrike \( \mathsf{M}_{n + 1} \),

\[  \mathsf{P}_{n + 1} - \mathsf{P}_n = \Phi_n \mathsf{P}_n \Phi_n^T + \Gamma_n Q_n \Gamma_n^T - \mathsf{M}_{n + 1} \mathsf{H}^T \left( \mathsf{H} \mathsf{M}_{n + 1}^T + \mathsf{R}_{n + 1} \right)^{-1} \mathsf{H} \mathsf{M}_{n + 1} - \mathsf{P}_n.
\]

Delimo s \( T \) in limitiramo \( T \to 0 \),

\begin{align*}
\lim_{T \to 0}  \frac{\mathsf{P}_{n + 1} - \mathsf{P}_n}{T} &= \lim_{T \to 0}  \left(\Phi_n \mathsf{P}_n \Phi_n^T + \Gamma_n Q_n \Gamma_n^T - \mathsf{M}_{n + 1} \mathsf{H}^T \left( \mathsf{H} \mathsf{M}_{n + 1}^T + \mathsf{R}_{n + 1} \right)^{-1} \mathsf{H} \mathsf{M}_{n + 1} - \mathsf{P}_n \right) \\
&= \lim_{T \to 0} \left( \frac{\Phi_n \mathsf{P}_n \Phi_n^T - \mathsf{P}_n}{T} \right) - \lim_{T \to 0} \left[ \mathsf{M}_{n + 1} \mathsf{H}^T \left( \mathsf{H} \mathsf{M}_{n + 1}^T T + \mathsf{R}_{n + 1}T \right)^{-1} \mathsf{H} \mathsf{M}_{n + 1}^T \right] + \\
&\quad + \lim_{T \to 0} \left( \frac{\Gamma_n Q_n \Gamma_n^T }{T}\right)
\end{align*}

Pri prvem členu upoštevamo definicijo \eqref{ali:3} in zapišemo

\begin{align*}
  \lim_{T \to 0} \left( \frac{\Phi_n \mathsf{P}_n \Phi_n^T - \mathsf{P}_n}{T} \right) &= \lim_{T \to 0} \left[\frac{\left( 1 + A T \right) \mathsf{P}_n \left( 1 + A^T T \right) - \mathsf{P}_n }{T} \right] \\
&= \lim_{T \to 0} \left[ \frac{\mathsf{P}_n + A \mathsf{P}_n T + \mathsf{P}_n A^T + A \mathsf{P}_n A^T T ^2 - \mathsf{P}_n}{T} \right] \\
&= A(t) \mathsf{P}(t) + \mathsf{P}(t) A^T (t).
\end{align*}

V drugem členu upoštevamo limiti \( \lim_{T \to 0} \mathsf{M}_{n + 1} = \mathsf{P} (t) \) ter \( \lim_{T \to 0} T \mathsf{R}_n = \mathsf{R}(t) \)

\[ \lim_{T \to 0} \left[ \mathsf{M}_{n + 1} \mathsf{H}^T \left( \mathsf{H} \mathsf{M}_{n + 1}^T T + \mathsf{R}_{n + 1}T \right)^{-1} \mathsf{H} \mathsf{M}_{n + 1}^T \right] = \mathsf{P} (t) \mathsf{H}^T \mathsf{R}^{-1} (t) \mathsf{H} \mathsf{P}(t),
\]
saj gre prvi člen \( \mathsf{H} \mathsf{M}_{n + 1}^T T \) v limiti v \( 0 \). Preostane nam še tretji člen, v katerem upoštevamo limiti \( \lim_{T \to 0}  \frac{\Gamma_n}{T} = \Gamma \) ter \( \lim_{T \to 0} \frac{\delta_{n n'}}{T} Q_n T = Q(t) \delta (t - t') \)

\begin{align*}
  \lim_{T \to 0} \left( \frac{\Gamma_n Q_n \Gamma_n^T }{T}\right) &= \lim_{T \to 0}  \left( \frac{\Gamma_n}{T} Q_n T \frac{\Gamma_n^T}{T} \right) \\
&= \Gamma Q(t) \Gamma^T,
\end{align*}
ki za beli šum divergira pri \( t = t' \).

Kovarianca je tako

\[ \dot{\mathsf{P}} (t) = A(t) \mathsf{P}(t) + \mathsf{P}(t) A^T (t) + \Gamma Q (t) \Gamma^T  - \mathsf{P}(t) \mathsf{H}^T \mathsf{R}^{-1} (t) \mathsf{H} \mathsf{P}(t).
\]
#+end_izpeljava
...
#+begin_idea
Obravnavamo enodimenzionalni primer delca z maso \( m \), radijem \( r \), ki se v viskozni tekočini z viskoznostjo \( \eta \) premika s hitrostjo \( v = \dot{x} \). Dinamiko delca opisuje drugi Newtonov zakon s Stokesovo silo upora ter naključne sile \( F_x (t) \), ki predstavljajo trke med delci in so za nas šum. Ob času \( t = 0 \) ne sprejemamo signala iz senzorjev, zato je \( R \to \infty \). Vrednost kovariančne matrike naj bo \( P(t = 0) = P_0 \) in \( \bar{x} = \bar{v} = 0 \). Dinamika v sistemu \( S \) je

\[ \frac{\mathrm{d} }{\mathrm{d} t } \begin{bmatrix} x \\ v \end{bmatrix} = \begin{bmatrix}
                                                                            0 & 1 \\
                                                                            0 & -\frac{1}{\tau}
                                                                             \end{bmatrix} \begin{bmatrix} x \\ v \end{bmatrix} + 0 + \begin{bmatrix} 0 \\ 1 \end{bmatrix} \frac{F_x (t)}{m},
\]
kjer je \( \tau^{-1} = 6 \pi r \eta \) in \( \left\langle \frac{F_x(t)}{m} \frac{F_x (t')}{m} \right\rangle = \delta(t - t') Q \). Iz enačbe \eqref{eq:20} lahko razberemo, kaj so matrika \( A \), vektor \( \mathbf{c} \) in dinamični šum.

Pripadajoče spreminjanje kovariančne matrike podaja enačba

\[ \dot{\mathsf{P}} = \begin{bmatrix}
                      2 \left\langle xv \right\rangle  & \left\langle v ^2 \right\rangle - \frac{1}{\tau} \left\langle xv \right\rangle\\
                      \left\langle v ^2 \right\rangle - \frac{1}{\tau} \left\langle xv \right\rangle & - \frac{2}{\tau} \left\langle v ^2 \right\rangle
                       \end{bmatrix} + \begin{bmatrix}
                                       0 & 0 \\
                                       0 & Q
                                        \end{bmatrix}.
\]
Pričakovane vrednosti \( \left\langle v ^2 \right\rangle, \ \left\langle xv \right\rangle\) se ustalijo, ko rešimo enačbo \( \dot{\mathsf{P}} = 0 \), medtem ko \( \left\langle x ^2 \right\rangle \) narašča linearno s časom.

Če začnemo meriti lego delca, je senzorsko okno oblike \( H = [1 \ 0] \) in kovariančna matrika ima sedaj obliko
...
\[ \dot{\mathsf{P}} = \begin{bmatrix}
                      ...
                      \left\langle v ^2 \right\rangle - \frac{1}{\tau} \left\langle xv \right\rangle - \frac{1}{R} \left\langle x ^2 \right\rangle \left\langle xv \right\rangle & - \frac{2}{\tau} \left\langle v ^2 \right\rangle - \frac{1}{R} \left\langle xv \right\rangle ^2 + Q
                       \end{bmatrix}.
\]
Stacionarna rešitev zgornje enačbe je enačba 4. stopnje

\[ y ^4 - 4 y ^3 - 4 y ^2 + \frac{4 \tau ^2 Q}{R} = 0, \ y = \frac{\tau}{R} \left\langle x ^2 \right\rangle,
\]
ki jo rešimo grafično in nam omeji prostor nedoločenosti lege! Če občasno merimo - groba meritev ali velik \( R \) - so vsi \( y \) majhni in sledi, da je negotovost kraja konstantna z

\[ \left\langle x ^2 \right\rangle _{\infty} = \tau \sqrt{R Q}.
\]
#+end_idea

#+attr_latex: :options [Dinamika]
#+begin_izpeljava
Delec z maso \( m \), radijem \( r \) in hitrostjo \( v = \dot{x} \) se giblje v tekočini z viskoznostjo \( \eta \). Nanjega deluje Stokesova sila \( F_s = 6 \pi r \eta v \) ter naključne sile \( F_x (t) \), ki predstavljajo trke med delci in so za nas šum. Dinamična enačba je 2. Newtonov zakon

\[ m \ddot{x} = - 6 \pi r \eta \dot{x} + F_x (t),
\]
kjer za šum predpostavimo konstanten \( Q \)

\[ \left\langle \frac{F_x(t)}{m} \frac{F_x (t')}{m} \right\rangle = \delta (t - t') Q.
\]
Problem, ki ga imamo, je to, da je zgornja enačba diferencialna enačba 2. reda, medtem ko smo do sedaj našo dinamiko opisovali z enačbi \eqref{eq:20}, ki je 1. reda. Rešimo ga tako, da 2. Newtonov zakon razdelimo na dve enačbi prvega reda s pomočjo hitrosti \( \dot{x} = v \). Torej

\begin{align*}
  \dot{x} &= v \\
\dot{v} &= - \frac{1}{\tau} v + \frac{F_x (t)}{m}, \quad \tau^{-1} = \frac{6 \pi r \eta}{m}.
\end{align*}
Zgornji sistem lahko sedaj zapišemo vektorsko za vektor \( \mathbf{x} = [x, v]^T \) kot

\[ \frac{\mathrm{d} }{\mathrm{d} t} \begin{bmatrix} x \\ v \end{bmatrix} = \begin{bmatrix}
                                                                           0 & 1 \\
                                                                           0 & -\tau^{-1}
                                                                            \end{bmatrix} \begin{bmatrix} x \\ v \end{bmatrix} + \begin{bmatrix} 0 \\ 1 \end{bmatrix} \frac{F_x (t)}{m}.
\]
Zgornji zapis primerjamo z \eqref{eq:20} in razberemo
...
\label{eq:22}
 A(t) = \begin{bmatrix}
          0 & 1 \\
          0 & -\tau^{-1}
           \end{bmatrix}, \quad \mathbf{c}(t) = \vec{0}, \quad \Gamma \mathbf{w} = \frac{F_x (t)}{m} \begin{bmatrix} 0 \\ 1 \end{bmatrix} 
\end{equation}
#+end_izpeljava

#+attr_latex: :options [Kovariančna matrika]
#+begin_izpeljava
Ob času \( t = 0 \) prenehamo meritev lege in naj ima takrat kovariančna matrika vrednost \( \mathsf{P}(t = 0) = P_0 \). Za čase \( t > 0 \) se matrika \( \mathsf{P} \) spreminja po enačbi

\begin{equation}
\label{eq:23}
 \dot{\mathsf{P}} = A(t) \mathsf{P}(t) + \mathsf{P}(t) A^T (t) + \Gamma Q(t) \Gamma^T - \mathsf{P} \mathsf{H}^T \mathsf{R}^{-1} \mathsf{H} \mathsf{P},
\end{equation}
vendar ker *ni dotoka novih meritev* gre \( \mathsf{R} \to \infty \). S tem sporočamo, da meritve senzorja nimajo vpliva na sistem \( M \). Enačba je torej

\[ \dot{\mathsf{P}} = A(t) \mathsf{P}(t) + \mathsf{P}(t) A^T (t) + \Gamma Q(t) \Gamma^T.
\]
Matrika \( \mathsf{P} \) je simetrična, zato velja \( \mathsf{P} A^T = \left( AP \right)^T \). Iz \eqref{eq:22} poračunamo
                                      ...
                                      \end{bmatrix} + \left( \begin{bmatrix}
                                                             ...
                                                             0 & - \frac{1}{\tau} 
                                                              \end{bmatrix} \begin{bmatrix}
                                                                            \left\langle x ^2 \right\rangle & \left\langle xv \right\rangle \\
                                                                             \left\langle xv \right\rangle & \left\langle v ^2 \right\rangle 
                                                                             \end{bmatrix}\right)^T + \begin{bmatrix} 0 \\ 1 \end{bmatrix} Q [0 \ 1] \\
&= \begin{bmatrix}
   2 \left\langle xv \right\rangle & \left\langle v ^2 \right\rangle - \frac{1}{\tau} \left\langle xv \right\rangle\\
   \left\langle v ^2 \right\rangle - \frac{1}{\tau} \left\langle xv \right\rangle & - \frac{2}{\tau} \left\langle v ^2 + Q \right\rangle
    \end{bmatrix}
\end{align*}

Drugače zapisano, smo dobili sistem enačb
...
Z rešitvijo tega sistema enačb lahko določimo ali se nedoločenosti \( \left\langle x ^2 \right\rangle, \left\langle xv \right\rangle \) in \( \left\langle v ^2 \right\rangle \) ustalijo, oz. \( \dot{\mathsf{P}} = 0 \). Rešimo za začetek enačbo za nedoločenost hitrosti:

\begin{align*}
  \frac{\mathrm{d} }{\mathrm{d} t}\left\langle v ^2 \right\rangle = 0 &= - \frac{2}{\tau} \left\langle v ^2 \right\rangle + Q \\
\left\langle v ^2 \right\rangle_{\infty} &= \frac{Q \tau}{2}.
\end{align*}
Nedoločenost hitrosti je konstantna. Povežemo jo lahko tudi s temperaturo tekočine preko ekviparticijskega izreka in določimo vrednost dinamičnega šuma \( Q \)

\[ \frac{1}{2} m \left\langle v ^2 \right\rangle _{\infty} = \frac{k T}{2} \implies Q = \frac{2 k T}{m \tau}.
\]
Sedaj lahko rešimo drugo enačbo

\[ \frac{\mathrm{d} }{\mathrm{d} t}\left\langle xv \right\rangle = 0 = \left\langle v ^2 \right\rangle - \frac{1}{\tau} \left\langle xv \right\rangle,
\]
v kateri upoštevamo rešitev prve enačbe:

\[ \left\langle v ^2 \right\rangle _{\infty} - \frac{1}{\tau} \left\langle xv \right\rangle_{\infty} \implies \left\langle xv \right\rangle_{\infty} = \frac{k T \tau}{m},
\]
in je tudi konstantna. Rešimo še tretjo enačbo, nedoločenost lege, ki nima stacionarne rešitve, saj je pogojena z zgornjim rezultatom

\[ \frac{\mathrm{d} }{\mathrm{d} t}\left\langle x ^2 \right\rangle = 2 \left\langle xv \right\rangle_{\infty} \ne 0.
\]
Po integraciji do časa \( t \) dobimo z uvedbo difuzijske konstante \( D = \frac{kT \tau}{m} \) difuzijsko enačbo

\[ \left\langle x ^2 (t) \right\rangle - \left\langle x ^2 (0) \right\rangle = \frac{2k T \tau}{m} t \implies \left\langle x ^2 \right\rangle = \left\langle x ^2(0) \right\rangle + 2 D t
\]

Negotovost lege torej narašča linearno s časom.
#+end_izpeljava

#+attr_latex: :options [Dodatne meritve]
#+begin_izpeljava
Sedaj ponovno začnemo meriti lego, kar upoštevamo s senzorskim oknom \( H = [1 \ 0] \) in kovariančno matriko izračunamo sedaj po polni enačbi \eqref{eq:23}. Dodaten člen, ki ga prejle še nismo upoštevali je 

\begin{align*}
  \mathsf{P} \mathsf{H}^T R^{-1} \mathsf{H} \mathsf{P} &= \begin{bmatrix}
                                                          \left\langle x ^2 \right\rangle & \left\langle xv \right\rangle \\
                                                          \left\langle xv \right\rangle & \left\langle v ^2 \right\rangle
                                                           \end{bmatrix} \begin{bmatrix} 1 \\ 0 \end{bmatrix} \frac{1}{R} [1 \ 0] \begin{bmatrix}
                                                                                                                                  ...
                                                                                                                                  \left\langle xv \right\rangle & \left\langle v ^2 \right\rangle
                                                                                                                                   \end{bmatrix} \\
&= \frac{1}{R} \begin{bmatrix} \left\langle x ^2 \right\rangle \\ \left\langle xv \right\rangle \end{bmatrix} \left[ \left\langle x ^2 \right\rangle \ \left\langle xv \right\rangle \right] \\
...
Z dodanim členom je spreminjanje kovariančne matrike določeno z
...
Ponovno iščemo stacionarno rešitev \( \dot{\mathsf{P}} = 0 \) za naslednji sistem treh enačb

\begin{align} \label{ali:p1}
  2 \left\langle xv \right\rangle - \frac{1}{R} \left\langle x ^2 \right\rangle ^2 &= 0 \\ \label{ali:p2}
\left\langle v ^2 \right\rangle - \frac{1}{\tau} \left\langle xv \right\rangle - \frac{1}{R} \left\langle x ^2 \right\rangle \left\langle xv \right\rangle &= 0 \\ \label{ali:p3}
-\frac{2}{\tau} \left\langle v ^2 \right\rangle - \frac{1}{R} \left\langle xv \right\rangle ^2 + Q &= 0,
\end{align}
ki ga rešimo tako, da iz enačbe \eqref{ali:p1} izrazimo

\[ \left\langle xv \right\rangle = \frac{1}{2R} \left\langle x ^2 \right\rangle ^2,
\]
in ga nesemo v enačbo \eqref{ali:p3}

\begin{align*}
  \left\langle v ^2 \right\rangle &= \frac{Q\tau}{2} - \frac{\tau}{2 R} \left\langle xv \right\rangle ^2 \\
&= \frac{Q \tau}{2} - \frac{\tau ^4 \left\langle x ^2 \right\rangle ^4 R}{2 ^3 R^4 \tau ^3},
\end{align*}
kjer smo zadnjo vrstico razširili z \( R \). Uvedemo brezdimenzijsko spremenljivko \( y = \left\langle x ^2 \right\rangle \frac{\tau}{R} \) in enačbo zapišemo kot

\[ \left\langle v ^2 \right\rangle = \frac{Q \tau}{2} - \frac{R}{8 \tau ^3} y ^4.
\]
Slednje nesemo v enačbo \eqref{ali:p2} in dobimo enačbo 4. reda

\[ y ^4 + 4 y ^3 + 4 y ^2 = \frac{4 Q T ^4}{R},
\]
ki jo rešimo grafično.

#+attr_latex: :width 0.5\textwidth :caption \caption{Grafična rešitev enačbe \cite{likar_osnove_2016}}
[[file:figures/brown.png]]

Če meritev opravljamo redko - groba meritev ali velik \( R \) - potem velja \( y \gg y^i, \ i = 2, 3, 4 \) in nedoločenost lege je konstantna

\[ y \approx \tau ^2 \sqrt{\frac{Q}{R}} = \frac{\tau}{R} \left\langle x ^2 \right\rangle_{\infty} => \left\langle x ^2 \right\rangle = \tau \sqrt{R Q}.
\]
#+end_izpeljava
** Primer: praznenje RC člena  
#+begin_idea
Imamo vezje s kondenzatorjem kapacitete \( C \) ter upor z upornostjo \( R \). Iz Kirchoffovega zakona \( \sum\limits_i^{} U_i = 0 \) izračunamo dinamiko v sistemu \( S \)

\[ \dot{U} = - \frac{1}{\tau} U + w(t), \ \frac{1}{\tau} = \frac{1}{RC}, 
\]
kjer je \( U \) napetost na kondenzatorju in za šum \( w(t) \) velja

\[ \left\langle w(t) w(t') \right\rangle = \delta(t - t') Q.
\]
Razberemo koeficiente Kalmanovega filtra \( A = - \tau^{-1} \) in \( \Gamma = 1 \).

Meritev napetosti na kondenzatorju ima obliko

\[ z = U + r, \ \left\langle r(t) r(t') \right\rangle = \delta(t - t') R.
\]
Začetna vrednost disperzije naj bo \( P_0 \), ob kasnejših časih pa jo določa enačba

\[ \dot{P} = - \frac{2}{\tau} P + Q - \frac{P ^2}{R}. 
\]

Stacionarna rešitev \( t \to \infty \) zgornje enačbe je za \( \dot{P} = 0 \). Rešujem kvadratno enačbo

\[ \frac{P ^2}{R} + \frac{2}{\tau} P - Q = 0,
\]
z rešitvama je

\begin{equation}
\label{eq:21}
 P_{1, 2 \infty} = - \frac{R}{\tau} \pm \frac{R}{\tau} \sqrt{1 + \frac{Q \tau ^2}{R}} 
\end{equation}

Do splošne rešitve za poljuben čas \( t \) pridemo s parcialnimi ulomki in znaša

\[ P(t) = - \frac{R}{\tau} + \alpha \left( \frac{1 + \exp \left\{ -\frac{2 \alpha}{R}t \right\}}{1 - \exp \left\{ - \frac{2\alpha}{R} t \right\}} \right), \ \alpha = \frac{R}{\tau} \sqrt{1 + \frac{Q \tau ^2}{R}}
\]

Za *majhen dinamičen šum* \( Q \) lahko \eqref{eq:21} razvijemo s Taylorjevim razvojem in izračunamo *konstanten* ojačevalni faktor

\[ K_{\infty} = \frac{P_{\infty}}{R} = \frac{\tau}{2} \frac{Q}{R}.
\]

V splošnem je ojačevalni faktor odvisen od časa. Dinamična enačba v sistemu \( M \) je

\[ \dot{\hat{U}} = - \frac{\hat{U}}{\tau} + K(t) \left( z - \hat{U} \right),
\]
za konstanten ojačevalni faktor pa je

\[ \dot{\hat{U}} = - \frac{\hat{U}}{\tau} + \frac{\tau Q}{2 R} \left( z - \hat{U} \right),
\]
kjer lahko uvedemo novo konstanto

\[ \dot{\hat{U}} = \frac{1}{\tau_{eff}} \hat{U} + \frac{Q \tau}{2 R}, \ \frac{1}{\tau_{eff}} = \frac{1}{\tau} + \frac{\tau Q}{2R},
\]
in velja \( \tau_{eff} < \tau \).

Če sunku, ki je močno zašumljen, sledimo s časovno odvisnim ojačevalnim faktorjem, popolnoma zajamemo pravi signal eksponentnega padanja kot je prikazano na spodnji sliki. Predvidevam (piše ne), da je tak ojačevalni faktor računsko zahtevnejši.

#+attr_latex: :width 0.5\textwidth :caption \caption{Zajemanje podatkov s časovno odvisnim ojačevalnim faktorjem \cite{likar_osnove_2016}, str. 51.}
[[file:figures/rc_clen_k.png]]

Če sunku, ki je močno zašumljen, sledimo s konstantnim ojačevalnim faktorjem, imamo dva /trade-offa/. Lahko imamo majhno vrednost \( Q \), ki nam prinese sistematično napako pri začetnem merjenju, kar je dobro, če nas zanima končna vrednost.

#+attr_latex: :width 0.5\textwidth :caption \caption{Zajemanje podatkov s konstantnim ojačevalnim faktorjem in majhnim šumom \( Q \). Začetna sistematična napaka je velika \cite{likar_osnove_2016}, str. 52.}
[[file:figures/rc_clen_q_small.png]]

Za večjo vrednost \( Q \) zmanjšamo sistematično napako, ampak so kasnejše meritve bolj zašumljene. Odločamo se na case-to-case basis.

#+attr_latex: :width 0.5\textwidth :caption \caption{Zajemanje podatkov s konstantnim ojačevalnim faktorjem in večjim šumom \( Q \). Začetna sistematična napaka je manjša, vendar je celotna meritev bolj zašumljena \cite{likar_osnove_2016}, str. 52}
[[file:figures/rc_clen_q_big.png]]
#+end_idea

#+attr_latex: :options [Dinamična enačba]
#+begin_izpeljava
Napetost na kondenzatorju s kapaciteto \( C \) označimo z \( U_C \), napetost na uporniku z uporom \( R \) pa z \( U_R \). Iz Kirchoffovega zakona sledi

\[ \sum\limits_i^{} U_i = U_C + U_R = 0. 
\]

V enačbo vstavimo definicijo napetosti na kondenzatorju \( U_C = \frac{e}{C} \), nato pa jo odvajamo po času

\begin{align*}
  \frac{e}{C} &= -U_R && \left| \frac{\mathrm{d} }{\mathrm{d} t} \right.\\
\frac{\dot{e}}{C} &= - \dot{U}_R.
\end{align*}

Upoštevamo definicijo električnega toka \( I = \frac{\mathrm{d} e}{\mathrm{d} t} \) ter enakost iz Kirchoffovega zakona \( U_C = - U_R \), iz česar sledi

\[ \dot{U}_C = - \frac{I}{C}. 
\]
Električni tok izrazimo preko Ohmovega zakona \( I = \frac{U_R}{R} \)

\[ \dot{U}_C = \frac{U_R}{RC} = - \frac{U_C}{RC} = - \frac{U_C}{\tau}.
\]
Dinamiki dodamo še naključen šum \( w(t) \) in smo dobili dinamiko v sistemu \( S \)
...
V primerjavi s splošnim Kalmanovim filtrom \eqref{eq:20} odčitamo koeficiente \( A = \tau^{-1} \) in \( \Gamma = 1 \). S pomočjo tega zapišemo disperzijo

\begin{equation}
\label{eq:25}
 \dot{P} = 2 AP + \Gamma Q \Gamma - \frac{P ^2}{R} = -\frac{2}{\tau} P - \frac{P ^2}{R} + Q. 
\end{equation}
#+end_izpeljava

#+attr_latex: :options [Stacionarna rešitev]
#+begin_izpeljava
Stacionarna rešitev enačbe \eqref{eq:25} je za \( t \to \infty \), iz česar sledi kvadratna enačba
...
\[ \alpha =  \frac{R}{\tau} \sqrt{1 + \frac{Q \tau ^2}{R}}.
\]
#+end_izpeljava

#+attr_latex: :options [Splošna rešitev]
#+begin_izpeljava
Pri splošni rešitvi rešujemo enačbo oblike

\[ \frac{\mathrm{d} P }{\frac{P ^2}{R} + \frac{2P}{\tau} - Q} = - \mathrm{d} t, 
\]
ki jo s pomočjo rešitev stacionarne enačbi zapišemo v ničelni obliki

\[ \frac{\mathrm{d} P \cdot R}{\left( P + \frac{R}{\tau} - \alpha \right) \left( P + \frac{R}{\tau} + \alpha \right)} = - \mathrm{d} t. 
\]

Dobljeno razbijemo sedaj na parcialne ulomke
...
\[ - \int\limits_0^t \, \mathrm{d} t = \frac{R}{2 \alpha} \left[ \int\limits_{P_0}^P \frac{\mathrm{d} P}{\left( P + \frac{R}{\tau} - \alpha \right)} - \int\limits_{P_0}^P \frac{\mathrm{d} P}{\left( P + \frac{R}{\tau} + \alpha \right)} \right].
\]

Rešitev integracije je

\begin{align*}
  -\frac{2\alpha t}{R} &= \left. \left[ \ln \left( P + \frac{R}{\tau} - \alpha \right) - \ln \left( P + \frac{R}{\tau} + \alpha \right) \right] \right|_{P_0}^P \\
&= \left. \ln \left( \frac{P + \frac{R}{\tau} - \alpha}{P + \frac{R}{\tau} + \alpha} \right) \right|_{P_0}^P.
\end{align*}

Zapišemo

\[   \frac{P + \frac{R}{\tau} - \alpha}{P + \frac{R}{\tau} + \alpha} =   \frac{P_0 + \frac{R}{\tau} - \alpha}{P_0 + \frac{R}{\tau} + \alpha} \exp \left\{ -\frac{2 \alpha}{R} t \right\}.
\]

Ob času \( t \to 0 \) o sistemu ne vemo veliko, zato \( P_0 \to \infty \) in posledično gre faktor na desni strani enačbe proti \( 1 \). Preostane nam

\[ \left( P + \frac{R}{\tau} - \alpha \right) = \left( P + \frac{R}{\tau} + \alpha \right) \exp \left\{ - \frac{2\alpha}{R} t \right\}, 
\]
od koder izrazimo

\[ P(t) = - \frac{R}{\tau} + \alpha \left( \frac{1 + \exp \left\{ - \frac{2 \alpha}{R} t \right\}}{1 - \exp \left\{ - \frac{2 \alpha}{R} t \right\}}  \right).
\]

Preverimo lahko tudi limito \( t \to \infty \) in dobimo rezultat identičen stacionarni rešitvi.
#+end_izpeljava
