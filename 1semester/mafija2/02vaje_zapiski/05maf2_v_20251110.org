...
#+begin_problem
Iščemo temperaturni profil snovi s širino \( a \), ki se ohlaja z radiacijo. Pri \( x = 0 \) je snov izolirana, medtem ko seva pri \( x = a \). Zunanja okolica ima temperaturo \( T_1 \).
#+end_problem
...
\begin{align*}
 T \left( \vec{r}, t \right) = &\sum\limits_{n, m, l = 1}^{} \frac{8 (T_1 - T_0)}{\pi ^3 nml}  \left( 1 - (-1) ^n \right) \left( 1 - (-1)^m \right) \left( 1 - (-1) ^l \right) \\
& \cdot e^{- D \frac{\pi ^2}{a ^2} \left( n ^2 +m  ^2 + l ^2 \right) t} \sin \left( \frac{\pi n}{a}x \right) \sin \left( \frac{\pi m}{a} y \right) \sin \left( \frac{\pi l}{a} z \right) + T_0.
\end{align*}
