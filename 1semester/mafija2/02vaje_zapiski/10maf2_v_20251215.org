#+title: 10. vaje iz Matematične fizike 2
...
\begin{align*}
  T_1 - T_0 &= \sum\limits_{l = 1}^{\infty} C_l P_l (\cos \theta) && \left/ \int\limits_0^1 P_l (\cos \theta) \, \mathrm{d} \cos \theta \right. \\
...
\]
** Naloga 30 :the_power_of_sturm_liouvile:
...
\begin{align*}
 -c_{n, l} \left( \frac{\xi_n^{(l)}}{R} \right) ^2 \int\limits_0^R \int\limits_{-1}^1 j_l ^2 \left( \xi_n^{(l)} \frac{r}{R} \right) P_l ^2 \left( \cos \theta \right) r ^2 \, \mathrm{d} r \, \mathrm{d} (\cos \theta) &= \\
-\frac{P}{\pi R ^2 \lambda} \int\limits_0^R \int\limits_{-1}^1 \frac{\delta (r \cos \theta)}{r} r ^2 j_l \left( \xi_n^{(l)} \frac{r}{R} \right) P_l (\cos \theta) \, \mathrm{d} r \, \mathrm{d} (\cos \theta) &= \\
