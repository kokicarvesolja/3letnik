...
Oglejmo si skalarni produkt \( \left\langle \psi \middle| \mathcal{T} \psi \right\rangle \). Če najdemo rešitev, lastno funkcijo, \( \left| \psi \right\rangle \), potem je tudi \( T \left| \psi \right\rangle \) rešitev z isto lastno energijo. Rešitvi sta tako ortogonalni in linearno neodvisni.

\begin{align*}
  \left\langle \psi \middle| T \psi \right\rangle &= \left\langle T \psi \middle| T (T \psi) \right\rangle^{\ast} \\
&= \left\langle T \psi \middle| T ^2 \psi \right\rangle^{\ast},
\end{align*}

kjer smo upoštevali definicijo antikomutativnosti. Za \( T = \mathrm{i} \sigma_y K \) velja

\[ T ^2 = \mathrm{i} \sigma_y K \mathrm{i} \sigma_y K = \mathrm{i} \sigma_y (-\mathrm{i}) (- \sigma_y) K ^2 = - \sigma_y ^2 = -I .
\]

Zgornji skalarni produkt tako postane

\[ \left\langle \psi \middle| T \psi \right\rangle = \left\langle T \psi \middle| - \psi \right\rangle^{\ast} = - \left\langle T \psi \middle| \psi \right\rangle^{\ast} = - \left\langle \psi \middle| T \psi \right\rangle.
\]

To je možno samo, če velja \( \left\langle \psi \middle| T \psi \right\rangle = 0 \).

Vsak energijski nivo je tako vsaj dvakrat degeneriran. Taki degeneraciji se reče Kramersova deneracija. Ortogonalen par \( \left| \psi \right\rangle \) in \( \mathcal{T} \left| \psi \right\rangle \) pa je Kramersov par.
