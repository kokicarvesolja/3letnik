...
Vrtilna količina ima tri komponente \( L_x, L_y \) in \( L_z \), ki med seboj ne komutirajo, saj velja \( \left[ L_{\alpha}, L_{\beta} \right]= \mathrm{i} \hbar \sum\limits_{\alpha \beta \gamma}^{} L_{\gamma}  \). Velikost vrtilne količin je enaka \( L ^2 = \vec{L} ^2 \), ki pa komutira s posameznimi komponentami \( \left[ L ^2, L_{\alpha} \right] = 0 \).
