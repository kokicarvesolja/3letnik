...
\[ A_i(\mathbf{r}) = \frac{\mu_0}{4 \pi r ^3} \int\limits_V^{} r_j  r'_j j_i (\mathbf{s}) \, \mathrm{d} ^3 \mathbf{s}.
\]
...
Dipolni člen zapišemo torej

\[ A_i (\mathbf{r}) = \frac{\mu_0}{8 \pi r ^3} r_j \int\limits_V^{} \left( j_i (\mathbf{r}') r'_j - r'_i j_j (\mathbf{r}') \right) \, \mathrm{d} ^3 \mathbf{r},
\]

oziroma v vektorski notaciji

\[ \mathbf{A}(\mathbf{r}) = \frac{\mu_0}{8 \pi r ^3} \int\limits_V^{} \left(\mathbf{j}(\mathbf{r}') \left( \mathbf{r} \cdot \mathbf{r}' - \mathbf{r} ' \left( \mathbf{j}(\mathbf{r}') \cdot \mathbf{r} \right) \right) \right) \, \mathrm{d} ^3 \mathbf{r}' = - \frac{\mu_0}{8 \pi r ^3} \mathbf{r} \times \int\limits_V^{} \left( \mathbf{r}' \times \mathbf{j}(\mathbf{r}') \right) \, \mathrm{d} ^3 \mathbf{r}'.
\]
...
\[ W = - \int\limits_{V_0}^{} \mathbf{j}(\mathbf{r}) \cdot \mathbf{A} (\mathbf{r}) \, \mathrm{d} ^3 \mathbf{r},
\]

kjer je \( V_0 \) končen volumen, v katerem se nahaja gostota toka \( \mathbf{j}(\mathbf{r}_0) \), \( \mathbf{A}(\mathbf{r}) \) pa je zunanje magnetno polje.

Magnetni potencial lahko razvijemo v okolici vektorja \( \mathbf{r}_0 \)

\[ \mathbf{A}(\mathbf{r}) = \mathbf{A}(\mathbf{r}_0) + \left( \left[ \mathbf{r} - \mathbf{r}_0 \right] \mathbf{A}(\mathbf{r}_0) \right) + \ldots
\]

Energija je potem

\[ W = - \int\limits_V^{} \mathbf{j}_0 (\mathbf{r}) \left[ \mathbf{A}(\mathbf{r}_0) + \left( \mathbf{r} - \mathbf{r}_0 \right) \cdot \nabla_{\mathbf{r}_0} \mathbf{A}(\mathbf{r}_0) + \ldots \right] \, \mathrm{d} ^3 \mathbf{r}
\]

Člen

\[ - \int\limits_V^{} \mathbf{j}_0 (\mathbf{r}) \mathbf{A}(\mathbf{r}_0) \, \mathrm{d} ^3 \mathbf{r} = 0,
\]

saj imamo sklenjene vse tokovne zanke.

Energijo dipolnega momenta lahko zapišemo v tenzorski notaciji

\[ \int\limits_V^{} \left( \mathbf{r} - \mathbf{r}_0 \right)\cdot \nabla_{\mathbf{r}_0} \mathbf{A}(\mathbf{r}_0) \, \mathrm{d} ^3 \mathbf{r} = \frac{\partial A_{j} (\mathbf{r}_0)}{\partial r_{0_i}} \int\limits_V^{} j_{0_j} \left( \mathbf{r} - \mathbf{r}_0 \right)_i \, \mathrm{d} ^3 \mathbf{r}
\]

Tukaj velja podobna identiteta kot pri razvoju potenciala

\[ \int\limits_V^{} \left( \mathbf{r} - \mathbf{r}_0 \right)_i j_{0_j} \, \mathrm{d} ^3 \mathbf{r} = - \int\limits_V^{} \left( \mathbf{r} - \mathbf{r}_0 \right)_j j_{0_i} \, \mathrm{d} ^3 \mathbf{r},
\]

in izraz se prepiše v

\begin{align*}
  W &= - \frac{\partial A_{j}(\mathbf{r}_0)}{\partial r_{0_i}} \frac{1}{2} \int\limits_V^{} \left( \left( \mathbf{r} - \mathbf{r}_0 \right)_i j_{0_j} - \left( \mathbf{r} - \mathbf{r}_0 \right)_j j_{0_i} \right) \\
&= - \frac{1}{2} \int\limits_V^{} \left[ \mathbf{j}_0 (\mathbf{r}) \left\{ \left( \mathbf{r} - \mathbf{r}_0 \right) \cdot \nabla_{\mathbf{r}_0} \right\} \mathbf{A}(\mathbf{r}_0) - \left( \mathbf{r} - \mathbf{r}_0 \right) \left( \mathbf{j}_0 (\mathbf{r}) \cdot \nabla_{\mathbf{r}_0} \right) \mathbf{A}(\mathbf{r}_0) \right] \, \mathrm{d} ^3 \mathbf{r}
\end{align*}

Uporabimo vektorsko identiteto

\[ \left( \mathbf{A} \times \mathbf{B} \right) \left( \mathbf{C} \times \mathbf{D} \right) = \left( \mathbf{A} \cdot \mathbf{C} \right) \left( \mathbf{B} \cdot \mathbf{D} \right) - \left( \mathbf{A} \cdot \mathbf{D} \right) \left( \mathbf{B} \cdot \mathbf{C} \right),
\]

energija dipolnega momenta pa tako postane

\[ W = - \left( \frac{1}{2} \int\limits_V^{} \left( \mathbf{r} - \mathbf{r}_0 \right) \times \mathbf{j}_0 (\mathbf{r}) \, \mathrm{d} ^3 \mathbf{r} \right) \cdot \nabla_{\mathbf{r}_0} \times \mathbf{A}(\mathbf{r}_0}) = - \mathbf{m} \cdot \mathbf{B}
\]
*** Sila in navor na magnetni dipol

Izpeljava za delo in navor je relativno podobna izpeljavi za električni dipol. Sprememba magnetne energije

\[ \mathrm{d} W = - \mathbf{F} \cdot \mathrm{d} \mathbf{r},
\]

kjer smo gostoto toka \( \mathbf{j} \) v zunanjem magnetnem polju \( \mathbf{B} \) premaknili za \( \mathrm{d} \mathbf{r} \).

Delo je v drugem redu multipolnega razvoje

\[ \mathrm{d} \left( \mathbf{m} \cdot \mathbf{B} \right) = \nabla \left( \mathbf{m} \cdot \mathbf{B} \right) \, \mathrm{d} \mathbf{r}.
\]

Upoštevamo identiteto

\[ \nabla \left( \mathbf{m} \cdot \mathbf{B} \right) = \mathbf{m} \times \left( \nabla \times \mathbf{B} \right) + \left( \mathbf{m} \cdot \nabla \right) \mathbf{B}.
\]

Sredinski člen je ničeln, saj ima zunanje magnetno polje \( \mathbf{B} \) gostote tokov \( \mathbf{j}' \), ki ga ustvarjajo, neskončno daleč stran. Rotor magnetnega polja je v tem primeru ničeln.

Sila je torej

\[ \mathbf{F} = \left( \mathbf{m} \cdot \nabla\right) \mathbf{B} (\mathbf{r}) .
\]

Delo pri navoru za zasuk \( \mathrm{d} \bm{\phi} \) je

\[ \mathrm{d} W = - \mathbf{M} \cdot \mathrm{d} \bm{\phi}.
\]

Zasuk magnetnega navora za \( \mathrm{d} \bm{\phi} \) je

\[ \mathrm{d} \mathbf{m} = \mathrm{d} \bm{\phi} \times \mathbf{m}.
\]

Delo je ponovno v drugem redu

\[ \mathrm{d} W = - \mathrm{d} \left( \mathbf{m} \cdot \mathbf{B}(\mathbf{r}) \right) = - \mathrm{d} \mathbf{m} \cdot \mathbf{B}(\mathbf{r}) = - \left( \mathrm{d} \bm{\phi} \times \mathbf{m} \right) \cdot \mathbf{B} (\mathbf{r}).
\]

Upoštevamo cikličnost mešanega produkta in primerjamo zgornji izraz za delo in delo z navorom ter dobimo
...
#+end_problem
*** Maxwellova formulacija EM indukcije in Maxwellov impulz

Faradayjev zakon indukcije

\[ \Gamma_E = - \frac{\mathrm{d} \Phi_{M}}{\mathrm{d} t}, 
\]

nam kvalitatitvno podaja zvezo, kako časovna sprememba magnetnega pretoka vpliva na električno cirkulacijo.

Ob upoštevanju definicije električne cirkulacije in magnetnega pretoka dobimo zvezo

\[ \oint\limits_C^{} \mathbf{E} \cdot \mathrm{d} \mathbf{r} = - \frac{\mathrm{d} }{\mathrm{d} t} \int\limits_S^{} \mathbf{B} \cdot \mathrm{d} \mathbf{S},
\]

kjer zanko opisuje krivulja \( C \), njena meja pa podaja površino \( S \). Preko Stokesovega izreka lahko integrala prevedemo na

\[ \int\limits_S^{} \nabla \times \mathbf{E} \cdot \mathrm{d} \mathbf{S} = - \int\limits_S^{} \frac{\partial B}{\partial t} \cdot \mathrm{d} \mathbf{S}.
\]

Predpostavili smo tudi, da se *zanka s časom ne spreminja*. Dobili smo kinematično Maxwellovo enačbo

\[ \nabla \times \mathbf{E} = - \frac{\partial \mathbf{B}}{\partial t}.
\]

Enačba ne vsebuje nobene konstante, ampak zgolj podaja direktno zvezo med prostorskimi odvodi električnega polja in časovnim odvodom magnetnega polja.

Iz zgornje kinematične enačbe in definicije vektorskega potenciala \( \mathbf{B} = \nabla \times \mathbf{A} \) lahko zapišemo

\[ \nabla \times \mathbf{E} = - \frac{\partial }{\partial t} \left( \nabla \times \mathbf{A} \right) = - \nabla \times \frac{\partial \mathbf{A}}{\partial t}.
\]

Za zaključene magnetne silnice velja v splošnem

\[ \mathbf{E} = - \frac{\partial \mathbf{A}}{\partial t}. 
\]

Faradayjev zakon indukcije lahko torej povežemo z drugim Newtonovim zakonom

\[ \mathbf{F} = e \mathbf{E} = - \frac{\partial e \mathbf{A}}{\partial t} = \frac{\mathrm{d} \mathbf{p}}{\mathrm{d} t},
\]

kjer je \( \mathbf{p} = e \mathbf{A} \) posplošeni impulz magnetnega polja. Impulz magnetnega polja predstavlja gibalno količino, ki jo ob indukciji dodajamo v sistem.
*** Kvazistatičen sistem Maxwellovih enačb in kontinuitetna enačba 

Kvazistatičen sistem Maxwellovih enačb so sestavljene iz štirih enačb:

- dve enačbi za izvor

  \[ \nabla \cdot \mathbf{E} = \frac{\rho}{\epsilon_0} \quad \text{ in } \quad \nabla \cdot \mathbf{B} = 0,
  \]

- ter dve enačbi za vrtince

  \[ \nabla \times \mathbf{E} = - \frac{\partial \mathbf{B}}{\partial t} \quad \text{ in } \quad \nabla \times \mathbf{B} = \mu_0 \mathbf{j}.  
  \]

Ta sistem (predvsem zadnja enačba) je pogojen s tem, da so tokovne zanke sklenjene, kar je s kontinuitetno enačbo podano kot

\[ \nabla \cdot \mathbf{j} = 0. 
\]
*** Elektromagnetna potenciala

Še vedno velja (in bo) veljalo

\[ \nabla \cdot \mathbf{B} = 0, 
\]

kar pomeni, da je uvedba vektorskega potenciala \( \mathbf{B} = \nabla \times \mathbf{A} \) korektna odločitev.

Iz Maxwellovega impulza pa sedaj sledi

\[ \nabla \times \mathbf{E} = - \nabla \times \frac{\partial \mathbf{A}}{\partial t} \implies \ \nabla \times \left( \mathbf{E} + \frac{\partial \mathbf{A}}{\partial t}  \right) = 0. 
\]

Električno polje mora biti še zmeraj brezvrtinčno in ta pogoj bo veljal, če je električno polje definirano s skalarnim električnim potencialom ter časovnim odvodom vektorskega potenciala

\[ \mathbf{E} = \nabla \phi - \frac{\partial \mathbf{A}}{\partial t}. 
\]

Tak potencial ne vpliva na kvazistatičen sistem Maxwellovih enačb, saj še zmeraj velja
...
\[ \nabla \times \mathbf{E} = \nabla \times \left( \nabla \phi - \frac{\partial \mathbf{A}}{\partial t} \right) = \frac{\partial \left( \nabla \times \mathbf{A} \right)}{\partial t} = - \frac{\partial \mathbf{B}}{\partial t}.
\]
...
** 22. vprašanje
#+begin_problem
Ohranitveni zakoni Maxwellovih enačb: razloži in opiši ohranjanje energije, gibalne in vrtilne količine.
#+end_problem
* EMP v snovi
** 23. vprašanje
#+begin_problem
Razloži in opiši magnetno polje v snovi: vezan tok, magnetizacija, konstitutivna relacija, pomen polarizacije in klasifikacija snovi. 
#+end_problem
** 24. vprašanje
#+begin_problem
Razloži in opiši magnetno polje v snovi: vezan tok, magnetizacija, konstitutivna relacija, pomen magnetizacije in klasifikacija snovi. 
#+end_problem
** 25. vprašanje
#+begin_problem
...
** 27. vprašanje
#+begin_problem
Uvedi in razloži frekvenčno odvisnost dielektrične funkcie in Kramers-Kronigove relacije. V kakšni povezavi sta disipacija energije in dielektrična funkcija? Zapiši osnovne modele za frekvenčno odvisno dielektrično funkcijo (gibalna količina za vezan naboj, Debyjeva relaksacija, Lorentzova relaksacija, plazemska relaksacija) in razloži, kako sta povezana dielektrična funkcija in prevodnost. 
#+end_problem
* Hamiltonske metode v teoriji polja
** 28. vprašanje
#+begin_problem
Določi in razloži Lagrengeovo in Hamiltonovo funkcijo nabitega gibajočega delca v zunanjem električnem in magnetnem polju. Nato izpelji >se Schwartzschildovo invarianto. Nadalje dopolni Lagrangeovo funkcijo še s prispevki zaradi samega polja, ki ga nabiti delci (delec) ustvarjajo in zapiši pripadajoče Euler-Lagrangeve in Rieman-Lorentzove enačbe.
#+end_problem
** 29. vprašanje
#+begin_problem
Posebna teorija relativnosti: razloži transformacijo EM polj z Lorentzovo transformacijo, vlogo prostora Minkovskega in formulacijo štirivektorjev, štirivektorja gostote toka in štirivektorja EM potenciala.
#+end_problem
