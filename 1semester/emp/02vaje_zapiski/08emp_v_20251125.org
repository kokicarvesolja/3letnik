...
in nam bo služil kot inspiracija modificirano Maxwellovo enačbo, kjer smo ji dodali člen \( \mu_0 \vec{\jmath}_m \), kjer je \( \vec{\jmath}_m  \) gostota magnetnega toka ob predpostavki, da je obstajajo monopoli. Enačba se bo tako glasila
...
\[ L_{ij} = \frac{\mu_0}{4 \pi} \oiint\limits_{}^{} \frac{\mathrm{d} \vec{l}_i \cdot \mathrm{d} \vec{l}_j}{ \left| \vec{r}_i - \vec{r}_j \right|}.
\]
...
I_1ntegrand postavimo na isti imenovalec
