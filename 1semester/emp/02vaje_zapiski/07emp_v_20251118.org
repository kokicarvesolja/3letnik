...
Na plašč delujeta dve sili - magnetna sila, ki je posledica toka, ki teče po žili koaksialnega kabla. Druga sila, ki kaže navznoter, pa je posledica toka, ki teče po plašču in predstavlja površinsko napetost - kakor napiše Mastnak, str. 36, predstavljamo si, da je plašč sestavljen iz veliko vzporednih žic, po katerih teče tok in se med seboj privlačijo.
...
S pomočjo Amperovega zakon lahko določimo magnetno polje znotraj in zunaj plašča
...
Zunaj plašča, kjer se nahajamo na točki \( r > a \) (zelena črtkana črta), je totalen zaobjet tok \( I_{tot} = I_z - I_p = 0 \). Iz tega sledi, da je magnetno polje zunaj žile ničelno \( \vec{B} = 0 \). Znotraj žile, v točki \( r < a \) (modra črtkana črta), pa je zaobjet tok samo tok žile, iz česar sledi magnetno polje
...
Ker zunaj ni magnetnega polja, pomeni, da tudi ni magnetne sile. Majhno površino plašča \( \mathrm{d} S \) zapišemo kot

\[ \mathrm{d} S = a \mathrm{d} \phi l
\]

Ker integriramo po ploskvi, ki tesno objema površino plašča, velja, da sta magnetno polje \( \vec{B} \) in normala ploskve \( \vec{n} \) pravokotna. Iz tega sledi, da je njun skalarni produkt \( \vec{B} \cdot \vec{n} = 0 \). Splošna formula za magnetno silo
